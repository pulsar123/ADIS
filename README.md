...
targeted) of faint moving star-like objects in imaging sequences taken with a telescope and an astro
...
is performed, to find the moving objects ("clouds" in 4D space). The 4D space consists of the x,y coordinates of the pixel in the first image, and the motion vector coordinates (Jx,Jy). The code is using GPU acceleration to 
